...
public class IPMachinesTripplesImprover2 extends Solver implements TweakOperator, Setup, Heuristic {
	...
	private static Logger logger = Logger.getLogger(IPMachinesTripplesImprover2.class);
	...
	public IPMachinesTripplesImprover2() {
	}
	...
	public void setup(RoadefConfiguration configuration, String base) {
		...
		maxNumTriesWithoutImprovement = configuration.getInt(base + ".max_num_tries_without_improvement", 30);
	...
	public Solution tweak(Solution solution, Deadline deadline) {
		...
		while (!deadline.hasExpired()) {
			...
			long bestOptimisticImp3 = 0;
			int m3 = 0;
			for (int m = 0; m < problem.getNumMachines(); ++m) {
				long opt1 = ProblemUtils.computeOptimisticImprovementForMachines(ss, pair.m1, m);
				long opt2 = ProblemUtils.computeOptimisticImprovementForMachines(ss, pair.m2, m);
				if (Math.max(opt1, opt2) > bestOptimisticImp3) {
					bestOptimisticImp3 = Math.max(opt1, opt2);
					m3 = m;
				}
			}
			...
			processes.addAll(ss.processesInMachine[m3]);

			long priority = queue.getPriority(pair.m1, pair.m2);
			logger.trace("Candidates: " + pair.m1 + ", " + pair.m2 + ", " + m3 + " (optimistic=" + bestOptimisticImp3 + ", priority=" + priority
					+ ", numProcesses=" + ss.processesInMachine[pair.m1].size() + "+" + ss.processesInMachine[pair.m2].size() + "+" + ss.processesInMachine[m3].size()
			...
			ss = (SmartSolution) model.modifyAssignmentsForProcesses(problem, ss, processes.toIntArray(),
